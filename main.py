...
def create_resource(resource: Resource):
    with get_db() as conn:
        ...
        cursor.execute(
            ...
            (resource.titulo, resource.descricao, resource.tipo, resource.url, json.dumps(resource.tags))
        )
...
def update_resource(resource_id: int, resource: Resource):
    with get_db() as conn:
        conn.execute(
            ...
            (resource.titulo, resource.descricao, resource.tipo, resource.url, json.dumps(resource.tags), resource_id)
        )
        return {"id": resource_id, **resource.dict()}
